...
class FontScreen extends StatelessWidget {
  ...
  /*
  글자 내부 그라디언트 적용 컬러의 경우
  Theme으로 그라디언트를 설정하여 그라디언트 완성 커스텀 클래스나
  컬러디자인을 따로 만든 후 호출되지 않는 한 직접적으로 사용하는 것 지양.
  web에서는 그라디언트가 적용되는 것 처럼 보이지만
  어플에서는 적용되지 않음.

  정~~~~~~~~~말로 원해서 사용하고자 한다면
  simple_gradient_text or gradient_borders 와 유사한 모듈들 존재
  -> 이 모듈을 사용해서 텍스트 글자 내부에 그라디언트를 줄 수 있음.

  그라디언트의 경우 글자 color가 아니라 image로 들어가는 색상 설정이므로
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            // 그라디언트의 경우 모듈을 사용하여 적용하기. ShaderMask 기본 모델은 버그가 많음!
            ShaderMask(
