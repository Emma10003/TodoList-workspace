...
StatefulWidget -> 변경되는 데이터가 위젯에 존재하기 때문에
                  ...
                  내부적으로 데이터를 교체하거나
                  주기적으로 변경할 수 있는 상태가 들어있는 UI를 참조하는 형태

class 파일명 extends 클라이언트한테_보여줄_위젯화면{
  고객님 저희는 변수와 기능 이렇게 사용할 거예요.
  우선적으로 화면에서 기다리는 고객들에게 사용하겠다
  변수와 기능명칭 선언

  외부에서 이 파일을 사용할 경우, 선언한 변수나 기능에서
  반드시 데이터를 전달하거나 전달해야 할 변수와 기능명 선언
  데이터 내부가 null이어도 되는 변수와 기능명을 구분지으며
  선언된 모든 변수와 기능은 생성자에서 필수인지 아닌지 작성해야 함.
}
 */
...
class _AddTodoDialogState extends State<AddTodoDialog> {
  // 외부 파일에 상태를 전달하기 위한 원격 리모컨 컨트롤러 생성
